...
1. If user provides a file path to a PRD, read it from disk (e.g. `sed -n '1,200p' <path>` / `cat <path>`)
2. If user describes requirements verbally, capture them
3. If requirements are unclear or incomplete, clarify by asking the user directly in chat:
...
In Codex workflow, create/update the file via repo edits (e.g. `apply_patch`) so it is version-controlled and can be referenced by later TDD/implementation skills.
